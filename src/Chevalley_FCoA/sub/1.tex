...
\begin{dfn}[dfn_compSeq][$⟙^n a$][Composite of a finite sequence]
  ...
  % \See{dfn_LawComp, dfn_uds, dfn_neut}
  \tcblower
  Assume $s = \{⟨ 1, a ⟩, ⟨ 2, b ⟩, ⟨ 3, c ⟩, ⟨ 4, d ⟩\}$, $a, b, c, d ∈ A$ and $\Monoid{A; ⊤}$. Then, $\Monoid{\uds ⊤; ⊤}$ and $s \： [1, 4] → \uds ⊤$. Thus,
  \begin{thmlist}
    \item 
    \?
...
\begin{cnv}
  \begin{thmlist}
    \item \Syn{$⟙^{n} a = ⟙^{n} \cls{ ⟨ i, a_i ⟩ }{ i ∈ 𝜔 }$}
    \[
      ⟙_{i = 1}^{n} a_i
      \AND
      a_1 \T \dots \T a_n
    \]
    \item \Syn{$\bigplus_{i = 1}^{n} a_i$}
    \[
      ∑_{i = 1}^{n} a_i
    \]
    \item \Syn{$\bigcdot_{i = 1}^{n} a_i$}
    \[
      ∏_{i = 1}^{n} a_i
    \]
  \end{thmlist}
